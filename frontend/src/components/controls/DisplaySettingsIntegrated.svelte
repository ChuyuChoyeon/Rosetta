...
// ============ 主页：Bing 历史日期切换（idx 0=今日 7=7天前） ============
const HOME_BING_IDX_KEY = "rosetta_home_bing_idx";
let homeBingIdx = $state(0);
let homeBingInfo: {
	url: string;
	title: string;
	copyright: string;
...
// ============ 主页：切换指定 idx 的 Bing 壁纸（更新 Banner 叠加层 + 右下角 Bing meta） ============
function applyHomeBingToDom(url: string, title: string, copyright: string) {
	// 1) 在 wallpaper-wrapper 内叠加一张覆盖图片（因为 banner 内用了 ImageWrapper 打包，不好直接改 src）
	...
	if (wrapper) {
		...
		if (!overlay) {
			...
			overlay.id = "home-bing-overlay";
			overlay.alt = "Bing wallpaper overlay";
			...
			overlay.style.zIndex = isFullBgMode ? "1" : "6";
			overlay.style.opacity = "0";
			overlay.style.transition = "opacity 0.8s ease";
			overlay.style.pointerEvents = "none";
			// 放到 #wallpaper-wrapper 第一个子节点之前（或直接 append）
	...
	// 2) 同步更新全局 Bing meta（MainGridLayout 右下角客户端脚本在读）
	try {
		(window as any).__bingWallpaperTitle = title || "";
		(window as any).__bingWallpaperCopyright = copyright || "";
		const evt = new CustomEvent("rosetta:bing-meta-change", {
			detail: { title, copyright },
		});
	...
	// 3) 直接写右下角 #bing-wallpaper-meta（若存在）
	const bm = document.getElementById("bing-wallpaper-meta");
	if (bm && (title || copyright)) {
		const inner = bm.querySelector(".bing-wallpaper-meta-inner");
		if (inner) {
			const titleEl = inner.querySelector(".bing-wallpaper-title");
			const copyEl = inner.querySelector(".bing-wallpaper-copyright");
			if (titleEl) (titleEl as HTMLElement).textContent = title;
			if (copyEl) (copyEl as HTMLElement).textContent = copyright;
		} else {
			// SSR / 初始占位或无 inner 容器的情况：客户端直接生成 inner
			let html =
			...
			if (title)
				html += `<div class="bing-wallpaper-title font-medium truncate">${String(title).replace(/"/g, "&quot;")}</div>`;
			if (copyright)
				html += `<div class="bing-wallpaper-copyright text-white/70 truncate text-[10px] md:text-xs">${String(copyright).replace(/"/g, "&quot;")}</div>`;
			html += "</div>";
			bm.innerHTML = html;
		}
		bm.style.opacity = "1";
...
async function applyHomeBingByIdx(idx: number, persist = true) {
	const safeIdx = Math.max(0, Math.min(7, Number(idx) || 0));
	homeBingIdx = safeIdx;
	loadingBing = true;
	try {
		const r = await fetchBingWallpaperByIndex(safeIdx, "zh-CN");
		const url = r?.url || "";
		if (!url) return;
		homeBingInfo = {
			url,
		...
		applyHomeBingToDom(url, r.title || "", r.copyright || "");
		if (persist) {
			try {
				localStorage.setItem(HOME_BING_IDX_KEY, String(safeIdx));
			} catch (_) {
				/* ignore */
			}
		}
	} catch (e) {
		console.warn(`[DisplaySettings] apply home bing idx=${safeIdx} failed:`, e);
	} finally {
		loadingBing = false;
	}
}

function formatStartDateShort(s: string): string {
...
// 挂载时，若本地存过 homeBingIdx 则恢复（仅 Banner / Fullscreen 模式生效，其他模式不切换图）
onMount(() => {
	let startIdx = 0;
	try {
		const raw = localStorage.getItem(HOME_BING_IDX_KEY);
		if (raw) startIdx = Math.max(0, Math.min(7, Number.parseInt(raw, 10) || 0));
	} catch (_) {
		/* ignore */
	}
	homeBingIdx = startIdx;
	const isImageMode =
		wallpaperMode === WALLPAPER_BANNER ||
		wallpaperMode === WALLPAPER_FULLSCREEN;
	if (isImageMode && startIdx >= 0) {
		applyHomeBingByIdx(startIdx, false);
	}
});
...
<div id="display-setting" class="float-panel float-panel-closed absolute transition-all w-80 right-4 px-3 pt-0 pb-3 max-h-[80vh] overflow-y-auto">
	...
	{#if activeTab === "wallpaper"}
		...
		<!-- Bing 历史日期切换（仅 Banner / Fullscreen 壁纸模式可见） -->
		{#if wallpaperMode === WALLPAPER_BANNER || wallpaperMode === WALLPAPER_FULLSCREEN}
		<div class="mt-2">
			<div class="section-title">
				<span class="text-[0.78rem] tracking-[0.14em] opacity-70 text-(--btn-content)">
					BING DATES · 切换历史日期
				</span>
				{#if loadingBing}
				<span class="text-[0.7rem] opacity-70 text-(--btn-content)">Loading…</span>
				{/if}
			</div>
			<div class="flex items-center gap-2">
				<button
					type="button"
					class="btn-regular rounded-md h-9 w-9 inline-flex items-center justify-center active:scale-90 transition-all shrink-0"
					disabled={homeBingIdx >= 7}
					title="向前一天（更旧的 Bing 壁纸）"
					onclick={() => applyHomeBingByIdx(homeBingIdx + 1, true)}
				>
					<Icon icon="material-symbols:chevron-left-rounded" class="text-[1.2rem]"></Icon>
				</button>
				<button
					type="button"
					class="btn-regular rounded-md h-9 flex-1 px-3 inline-flex items-center justify-center gap-2 active:scale-95 transition-all text-sm overflow-hidden"
					title={homeBingIdx === 0 ? "已是今日 Bing 壁纸" : "点击跳回今日 Bing 壁纸"}
					onclick={() => applyHomeBingByIdx(0, true)}
				>
					<span class="font-semibold">Day {homeBingIdx}</span>
					{#if homeBingIdx === 0}
						<span class="text-xs opacity-75">· Today</span>
					{:else}
						<span class="text-xs opacity-75">· {homeBingIdx}d ago</span>
					{/if}
					{#if homeBingInfo?.startdate}
						<span class="truncate text-xs opacity-80">· {formatStartDateShort(homeBingInfo.startdate)}</span>
					{/if}
				</button>
				<button
					type="button"
					class="btn-regular rounded-md h-9 w-9 inline-flex items-center justify-center active:scale-90 transition-all shrink-0"
					disabled={homeBingIdx <= 0}
					title="向后一天（更新的 Bing 壁纸）"
					onclick={() => applyHomeBingByIdx(homeBingIdx - 1, true)}
				>
					<Icon icon="material-symbols:chevron-right-rounded" class="text-[1.2rem]"></Icon>
				</button>
			</div>
			{#if homeBingInfo && (homeBingInfo.title || homeBingInfo.copyright)}
			<div class="mt-2 rounded-md bg-(--btn-regular-bg) p-2 text-xs leading-snug">
				{#if homeBingInfo.title}
					<div class="font-medium text-(--btn-content) truncate">{homeBingInfo.title}</div>
				{/if}
				{#if homeBingInfo.copyright}
					<div class="mt-0.5 text-(--btn-content) opacity-75 truncate">{homeBingInfo.copyright}</div>
				{/if}
			</div>
			{/if}
		</div>
