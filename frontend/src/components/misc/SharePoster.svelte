...
let dynamicSiteTitle = "ROSETTA";
let dynamicSiteUrl = "";
let dynamicSiteDescription = "";
onMount(async () => {
	try {
		const { loadDynamicConfig } = await import("@/utils/dynamic-config");
		const cfg = await loadDynamicConfig();
		dynamicSiteTitle = cfg.site.title;
		dynamicSiteUrl = cfg.site.siteUrl;
		dynamicSiteDescription = cfg.site.description;
	} catch (e) {}
...
const siteConfig = {
	get title() {
		return dynamicSiteTitle;
	},
	get site_url() {
		return dynamicSiteUrl;
	},
	get description() {
		return dynamicSiteDescription;
	},
};
