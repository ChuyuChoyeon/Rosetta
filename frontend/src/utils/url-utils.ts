...
export function url(path: string): string {
	...
	// 只有本地相对路径才添加BASE_URL
	let normalized = joinUrl("", import.meta.env.BASE_URL, path);

	// trailingSlash: "always" 兼容：为站内页面型路径统一补齐末尾 /
	// 跳过已显式带文件扩展名的（.xml .jpg .png .webp .avif .svg .js .css .json .ico 等）
	const hasExt = /\.[a-zA-Z0-9]{1,6}(\?|#|$)/.test(normalized);
	if (!hasExt) {
		// 去掉末尾的 ?query 和 #hash，先处理主路径
		const qIdx = normalized.search(/[?#]/);
		const base = qIdx >= 0 ? normalized.slice(0, qIdx) : normalized;
		const tail = qIdx >= 0 ? normalized.slice(qIdx) : "";
		if (!base.endsWith("/")) {
