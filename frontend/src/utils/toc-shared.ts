...
/**
 * 统一 slug / anchor id 生成算法（中文 + Unicode 友好）
 * 算法：str.toLowerCase().replace(/[^\p{L}\p{N}]+/gu, '-').replace(/(^-|-$)/g, '')
 * 与 Astro / github-slugger 主流做法兼容，确保中文标题 slug 稳定可预测。
 */
...
export const slugify = toAnchorId;
