...
const { data, pending, refresh } = await useFetch<any>("/api/admin/guestbook", {
  query: computed(() => ({
    page: page.value,
    pageSize,
...
async function submitReply() {
  ...
  try {
    await apiPost(`/api/admin/guestbook/${replyTarget.value.id}/reply`, { content: replyContent.value });
...
async function updateStatus(id: number | string, newStatus: string) {
  try {
    await apiPatch(`/api/admin/guestbook/${id}`, { status: newStatus });
...
async function remove(id: number | string) {
  if (!confirm("确定删除该留言？此操作不可撤销。")) return;
  deleting.value = id;
  try {
...
<template>
  <div class="space-y-lg">
    <header class="flex flex-col md:flex-row md:items-center md:justify-between gap-sm">
      <div>
        <h1 class="text-2xl font-bold text-neutral-text-primary">留言管理</h1>
        <p v-if="data?.pendingCount" class="text-sm text-warning-500 mt-1">
          <UIcon name="material-symbols:mail-rounded" class="w-4 h-4 inline mr-0.5" />
          有 {{ data.pendingCount }} 条留言待审核
        </p>
      </div>
    </header>
    ...
    <form
      ...
      @submit.prevent="navigateTo({ path: '/admin/guestbook', query: { q: keyword || undefined, status: statusFilter, page: 1 } })"
    >
      <UInput v-model="keyword" type="search" placeholder="搜索昵称 / 内容" class="sm:flex-1" />
    ...
    <div class="bg-neutral-bg-container border border-neutral-border-secondary rounded-2xl shadow-sm overflow-hidden">
        ...
        <table class="w-full text-sm">
          <thead class="bg-neutral-fill-hover text-xs text-neutral-text-tertiary uppercase tracking-wider">
            <tr>
              ...
              <th class="px-4 py-3 text-left font-medium">留言内容</th>
              <th class="px-4 py-3 text-left font-medium">状态</th>
              <th class="px-4 py-3 text-left font-medium hidden sm:table-cell">时间</th>
              <th class="px-4 py-3 text-right font-medium w-56">操作</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-neutral-border-secondary">
            <tr v-for="item in (data?.items || [])" :key="item.id" class="hover:bg-neutral-fill-hover/40 transition-colors">
              ...
              <td class="px-4 py-3 align-top">
                <div class="flex items-center gap-sm">
                  <div class="w-9 h-9 rounded-lg bg-gradient-to-br from-primary-500 to-nebula-blue text-white flex items-center justify-center font-semibold text-sm flex-shrink-0">
                    {{ (item.nickname || '?').slice(0, 1) }}
                  </div>
                  <span class="font-medium text-neutral-text-primary">{{ item.nickname || '匿名' }}</span>
                </div>
              </td>
              <td class="px-4 py-3 align-top min-w-[250px]">
                <p class="text-neutral-text-secondary whitespace-pre-wrap line-clamp-4">{{ item.content }}</p>
              </td>
              <td class="px-4 py-3 align-top">
            ...
            <tr v-if="!pending && data?.items?.length === 0">
              <td colspan="6" class="px-4 py-12 text-center text-neutral-text-tertiary text-sm">
                <UIcon name="material-symbols:edit-note-rounded" class="w-12 h-12 mx-auto mb-2 text-neutral-text-quaternary" />
                <p>暂无留言</p>
              </td>
            </tr>
          </tbody>
      ...
      <div v-if="(data?.totalPages || 1) > 1" class="flex items-center justify-between px-4 py-3 border-t border-neutral-border-secondary text-sm">
        <NuxtLink
          v-if="page > 1"
          :to="{ path: '/admin/guestbook', query: { ...route.query, page: page - 1 } }"
    ...
    <UDialog v-model="replyOpen" title="回复留言" size="lg">
      <div v-if="replyTarget" class="space-y-sm">
        <div class="bg-neutral-fill-hover rounded-lg p-sm">
          <p class="text-xs text-neutral-text-tertiary mb-xs">
            <span class="font-medium text-neutral-text-secondary">{{ replyTarget.nickname || '匿名' }}</span>
