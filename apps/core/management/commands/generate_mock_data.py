...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--users", type=int, default=5, help="生成用户数量 (默认: 5)"
        )
        parser.add_argument(
            "--categories", type=int, default=5, help="生成分类数量 (默认: 5)"
        )
        parser.add_argument(
            "--tags", type=int, default=10, help="生成标签数量 (默认: 10)"
        )
        parser.add_argument(
            "--posts", type=int, default=20, help="生成文章数量 (默认: 20)"
        )
        parser.add_argument(
            "--comments", type=int, default=60, help="生成评论数量 (默认: 60)"
        )
        parser.add_argument(
            "--password", type=str, default="password123", help="默认用户密码"
        )
        parser.add_argument(
            "--no-extras", action="store_true", help="不生成附加数据（友链、导航等）"
        )
