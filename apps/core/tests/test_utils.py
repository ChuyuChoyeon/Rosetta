...
def test_run_image_queue(mock_process, mock_cache):
    ...
    with patch("blog.models.Post.objects") as mock_objects:
        mock_post = MagicMock()
        mock_post.id = 1
        mock_objects.filter.return_value.exclude.return_value.order_by.return_value.__getitem__.return_value = [
            mock_post
        ]
...
def test_run_media_scan(mock_walk, mock_get_models, mock_cache, settings):
    ...
    mock_model.objects.exclude.return_value.exclude.return_value.values_list.return_value.iterator.return_value = [
        "file1.jpg"
    ]
    mock_get_models.return_value = [mock_model]

    # Mock os.walk
    mock_walk.return_value = [("/media", [], ["file1.jpg", "file2.jpg", ".hidden"])]
...
def test_create_backup(mock_call_command, settings, tmp_path):
    ...
    settings.DATABASES = {
        "default": {"ENGINE": "django.db.backends.postgresql", "NAME": "rosetta"}
    }
