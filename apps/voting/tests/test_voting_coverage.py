...
class TestVotingExtras:
    ...
    def user(self, django_user_model):
        return django_user_model.objects.create_user(username="voter", password="password")
    ...
    def test_get_active_polls(self):
        ...
        p2 = Poll.objects.create(title="P2", is_active=True, end_date=now + timedelta(days=1))
        
        # 3. Active, past end date (should not be returned)
        p3 = Poll.objects.create(title="P3", is_active=True, end_date=now - timedelta(days=1))
