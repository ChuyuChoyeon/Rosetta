...
class BulkActionView(LoginRequiredMixin, StaffRequiredMixin, View):
    ...
    def post(self, request, model):
        ...
        try:
            Model = apps.get_model(model.split(".")[0], model.split(".")[1]) if "." in model else None
            ...
            if action == "delete":
                queryset.delete()
                messages.success(request, _("已删除 {count} 个项目").format(count=count))
            elif action == "published":
                if hasattr(Model, "status"):
                    queryset.update(status="published")
                    messages.success(request, _("已发布 {count} 个项目").format(count=count))
            elif action == "draft":
                if hasattr(Model, "status"):
                    queryset.update(status="draft")
                    messages.success(request, _("已设为草稿 {count} 个项目").format(count=count))
