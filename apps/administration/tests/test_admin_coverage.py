...
class TestTaskServices:
    ...
    def test_get_task_info_map(self, mock_app):
        ...
        mock_app.tasks = {"task1": task1, "task2": task2}
...
class TestLogViews:
    ...
    def test_logfile_download(self, admin_client):
        url = reverse(
            "administration:logfile_download", kwargs={"filename": "test.log"}
        )
        response = admin_client.get(url)
        assert response.status_code == 200
    ...
    def test_logfile_download_not_found(self, admin_client):
        url = reverse(
            "administration:logfile_download", kwargs={"filename": "missing.log"}
        )
        response = admin_client.get(url)
