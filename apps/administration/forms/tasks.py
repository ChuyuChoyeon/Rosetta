...
class PeriodicTaskForm(StyleFormMixin, forms.ModelForm):
    ...
    # Crontab 字段
    cron_minute = forms.CharField(label=_("分 (Minute)"), initial="*", required=False, widget=forms.TextInput(attrs={"placeholder": "*", "class": "input input-bordered w-full"}))
    cron_hour = forms.CharField(label=_("时 (Hour)"), initial="*", required=False, widget=forms.TextInput(attrs={"placeholder": "*", "class": "input input-bordered w-full"}))
    cron_day_of_week = forms.CharField(label=_("周 (Day of Week)"), initial="*", required=False, widget=forms.TextInput(attrs={"placeholder": "*", "class": "input input-bordered w-full"}))
    cron_day_of_month = forms.CharField(label=_("日 (Day of Month)"), initial="*", required=False, widget=forms.TextInput(attrs={"placeholder": "*", "class": "input input-bordered w-full"}))
    cron_month_of_year = forms.CharField(label=_("月 (Month of Year)"), initial="*", required=False, widget=forms.TextInput(attrs={"placeholder": "*", "class": "input input-bordered w-full"}))

    class Meta:
        model = PeriodicTask
        fields = [
            "name", "task", 
            "args", "kwargs", "queue", "priority", 
            "expires", "one_off", "start_time", "enabled", "description"
        ]
        labels = {
            "name": _("任务名称"),
            "task": _("Task (注册路径)"),
        ...
        widgets = {
            ...
            "args": forms.Textarea(attrs={"rows": 2, "placeholder": "[]", "class": "textarea textarea-bordered w-full font-mono"}),
            "kwargs": forms.Textarea(attrs={"rows": 2, "placeholder": "{}", "class": "textarea textarea-bordered w-full font-mono"}),
            "start_time": forms.DateTimeInput(attrs={"type": "datetime-local", "class": "input input-bordered w-full"}),
            "expires": forms.DateTimeInput(attrs={"type": "datetime-local", "class": "input input-bordered w-full"}),
            "name": forms.TextInput(attrs={"class": "input input-bordered w-full"}),
            "queue": forms.TextInput(attrs={"class": "input input-bordered w-full"}),
            "priority": forms.NumberInput(attrs={"class": "input input-bordered w-full"}),
            "enabled": forms.CheckboxInput(attrs={"class": "toggle toggle-primary"}),
            "one_off": forms.CheckboxInput(attrs={"class": "checkbox checkbox-primary"}),
        }
        help_texts = {
            "task": _("选择要执行的 Celery 任务。您可以参考 'core.tasks' 中的示例任务。"),
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 动态填充任务选择
        try:
            self.fields['task'].widget = forms.Select(choices=get_registered_tasks(), attrs={"class": "select select-bordered w-full"})
        except Exception:
            # 如果 Celery 未运行或其他错误，回退到文本输入
            self.fields['task'].widget = forms.TextInput(attrs={"class": "input input-bordered w-full"})
        ...
        if instance:
            if instance.interval:
                ...
                self.fields["cron_month_of_year"].initial = instance.crontab.month_of_year
