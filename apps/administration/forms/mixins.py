        ...
        for name, field in self.fields.items():
            ...
            if isinstance(widget, forms.CheckboxInput):
                ...
                if "file-input" not in css_class:
                    attrs.update({"class": css_class + " file-input file-input-bordered w-full"})
            elif isinstance(widget, forms.CheckboxSelectMultiple):
                pass # Special handling usually needed
            elif isinstance(widget, forms.Textarea):
                if "textarea" not in css_class:
                    attrs.update({"class": css_class + " textarea textarea-bordered w-full"})
            elif isinstance(widget, forms.Select):
                if "select" not in css_class:
                    attrs.update({"class": css_class + " select select-bordered w-full"})
            else:
                # Default input
