...
class MockDataGenerator:
    ...
    def _generate_markdown_content(self) -> str:
        ...
        items = "\n".join(
            [f"- **{self.fake.word()}**: {self.fake.sentence()}" for _ in range(6)]
        )
    ...
    def create_users(self, count=5, password="password123"):
        ...
        for _ in range(count):
            ...
            user = self.User.objects.create_user(
                ...
                github=(
                    f"https://github.com/{username}" if random.random() > 0.5 else ""
                ),
                date_joined=self.fake.date_time_this_year(
    ...
    def create_categories(self, count=5):
        ...
        potential_names = [
            name for name in tech_categories if name not in existing_names
        ]
    ...
    def create_posts(self, count=20, users=None, categories=None, tags=None):
        ...
        for _ in range(count):
            ...
            if tags:
                post.tags.set(
                    random.sample(tags, k=random.randint(1, min(5, len(tags))))
                )
        ...
        for _ in range(count):
            ...
            comment = Comment.objects.create(
                ...
                created_at=self.fake.date_time_between(
                    start_date=post.created_at, tzinfo=timezone.get_current_timezone()
                ),
                active=True,
            )
    ...
    def create_navigations(self):
        ...
        created_navs = []
        for item in navs:
            if not Navigation.objects.filter(
