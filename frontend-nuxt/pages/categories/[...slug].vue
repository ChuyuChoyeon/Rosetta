...
const category = computed(() => decodeURIComponent((route.params.slug as string[]).join("/")));
useHead({ title: `分类：${category.value} - Rosetta` });
const page = ref(parseInt(String(route.query.page || 1), 10) || 1);
const pageSize = 10;
const { data } = await useFetch<any>(() => "/api/posts", {
  query: computed(() => ({ category: category.value, page: page.value, pageSize, _timeout: 8000 })),
  default: () => ({ items: [], total: 0 }),
  lazy: true,
});
const { data: local } = await useAsyncData(() => "cat-" + category.value, () =>
  queryContent("/posts").where({ draft: { $ne: true }, category: category.value }).sort({ published: -1 }).limit(30).find()
);
...
<template>
  <div class="space-y-lg">
    <header class="flex items-baseline gap-xs">
      <h1 class="text-2xl font-bold text-neutral-text-primary">分类：</h1>
      <span class="px-3 py-1 rounded-full bg-nebula-blue text-white text-sm font-medium">{{ category }}</span>
    ...
    <div class="space-y-sm">
      ...
      >
        ...
        <p class="mt-xs text-sm text-neutral-text-secondary line-clamp-1">{{ p.description }}</p>
